...
@app_1.route("/brand/<int:id>", methods=["GET"])
...
@app_1.route("/product/<int:product_id>", methods=["GET"])
...
@app_2.route("/user/<int:id>", methods=["GET"])
def user(id):
    try:
        if id == users[id - 1]["id"]:
            return (
                jsonify(
                    {
                        "message": f"User {users[id -1]['name']} is Nr.{id} , Email: {users[id-1]['email']} "
                    }
                ),
                200,
            )
...
@app_2.route("/login/<int:id>", methods=["GET"])
def login(id):
    try:
        if id == users[id - 1]["id"]:
            return (
                jsonify(
                    {"message": f"User {users[id -1]['name']} successfully logged in"}
                ),
                200,
            )
        else:
